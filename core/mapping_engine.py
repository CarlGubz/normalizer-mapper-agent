...
def _assemble_outputs(
    ...
    # Positive confirmation of where each cross-reference file actually came from this
    # run — {filename: "blob"|"local"|"local_fallback"} — always present (even {} when
    # no AMT enrichment ran at all), so "is it really using Blob?" has a direct answer
    # in the response itself, not just an absence-of-warning inference. See
    # core/cross_reference.py's xref_sources().
    report["cross_reference_sources"] = cross_reference.xref_sources()

    # A degraded source (blob configured but unavailable for a specific file, silently
    # fell back to the bundled prompts/cross-references/ copy) additionally gets a
    # warning — worth an operator's attention; a plain "local" source (no blob
    # configured at all) is the normal zero-config case and isn't warning-worthy on its
    # own, so it's left out of `warnings` (it's still visible above, just not flagged).
    for xref_name, source in report["cross_reference_sources"].items():
