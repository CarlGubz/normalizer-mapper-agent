...
```json
{
  ...
  "mapping_report": {
    ...
    "cross_reference_sources": { "fmg_cross-reference.csv": "local" },
...
**Observability — "is it actually using Blob?"** Every response's `mapping_report`
carries a `cross_reference_sources` field: `{filename: "blob"|"local"|"local_fallback"}`
for every cross-reference file that run actually touched — a direct, positive answer
in the response itself, not something to infer from the absence of a warning:
```json
"cross_reference_sources": { "fmg_cross-reference.csv": "blob" }
```
`"blob"` = fetched from `CROSS_REFERENCE_BLOB_CONTAINER` successfully this run.
`"local"` = no container configured at all — the normal zero-config case. `"local_fallback"`
= the container **was** configured but this specific file couldn't be fetched from it,
so the bundled copy was used instead — this one *also* gets a `mapping_report.warnings`
entry, since it's the one case worth an operator's attention:
```
...
```
`"blob"`/`"local"` don't warn — they're both normal, expected outcomes, so warning on
them would just be noise. Verified end to end: pointed `CROSS_REFERENCE_BLOB_CONTAINER`
at a nonexistent container with unreachable Entra ID auth and confirmed (a) the run
still succeeded with the exact same row counts as the no-Blob baseline (proving the
local fallback recovered identical data, not degraded data), (b) `cross_reference_sources`
correctly read `"local_fallback"`, and (c) the warning above fired naming the correct
file and container. See `core/cross_reference.py`'s `_read_xref_bytes`/`xref_sources`
for the implementation.

## 16. Onboarding note — Thiess, Macmahon, MACA (2026-09)

Three new customers were onboarded from `test-data/04. Thiess/`, `test-data/05.
Macmahon/`, and `test-data/06. MACA/` — each folder was searched for a **customer file**
(the raw forecast/maintenance-plan workbook to run through the pipeline) and a
**relevant cross-reference file** (the AMT component-code lookup table), following the
same pattern as §15/`prompts/README.md`. The two identified cross-reference files were
converted to CSV and added to `prompts/cross-references/` (bundled fallback, same as
the existing three) — upload them to `CROSS_REFERENCE_BLOB_CONTAINER` under these exact
filenames if/when Blob-backed cross-reference lookup (§15) is configured for this
account; **that upload wasn't performed by this change** — no
`AZURE_STORAGE_CONNECTION_STRING`/`AZURE_STORAGE_ACCOUNT_URL` was configured in this
environment, so there was nothing to authenticate the upload against. The bundled local
copies work today with zero configuration (`cross_reference_sources` reports `"local"`
either way, per §15).

| Customer | Customer file | Cross-reference file | Config |
|---|---|---|---|
| Thiess (+ MACA, combined) | `New Westrac Thiess (WA) Supplier Plan_May 26_WORKFILE.xlsx` — **Supply Plan** (→NEO) + **Component History** (→LAO) sheets | `New Crossref Thiess & Maca 21082026.xlsx` → `thiess_cross-reference.csv` | `config/customers/thiess.json`, `prompts/appendix.thiess.md` |
| Macmahon | `JULY 2026 Macmahon WA Based - Westrac Monthly Caterpillar Equipment.xlsx` — single **Export** sheet, feeds both NEO and LAO | `MacMahon CrossRef New 21082026.xlsx` → `macmahon_cross-reference.csv` | `config/customers/macmahon.json`, `prompts/appendix.macmahon.md` |
| MACA | **none supplied** — see below | `Maca CrossRef.xlsx` → `maca_cross-reference.csv` | `config/customers/maca.json` (scaffold), `prompts/appendix.maca.md` |

**Why Thiess and MACA share one profile.** Both are managed through the same Westrac
workbook and the same cross-reference file, and the historically-produced reference
output (`Thiess & Maca NEO/LAO Upload May26.csv`) is itself one combined file
distinguishing Thiess from MACA purely by the resolved `CustomerCode` value per row —
not two separate exports. `customer_id: "thiess"` reproduces that shape directly rather
than splitting it into two profiles that would run against the identical workbook.

**MACA has no source workbook in test-data** — only its cross-reference file and the
two already-finalized `Maca NEO/LAO Upload May26.csv` reference files were supplied.
This isn't a stand-in for a missing file: the MACA-tagged rows inside the Thiess
workbook's own `AMT` tab (`Customer == "MACA MINING PTY LTD"`, 2,466 rows) share **zero**
assets with the standalone Maca upload files (55 distinct assets there vs. 22 in the
Thiess-embedded rows) and cover different sites entirely — confirming MACA is a
genuinely separate data feed, not derivable from the Thiess workbook. `maca.json` and
`maca_cross-reference.csv` are onboarded and ready, but `core/cross_reference.py`'s
`ENRICHERS` deliberately has **no** `("maca", ...)` entry yet — writing one without a
real workbook to check its join key's ambiguity rate against (the way `thiess_neo` and
`macmahon_neo` below were) would be unverifiable guesswork. See
`prompts/appendix.maca.md` for exactly what's needed to finish this once MACA's real
monthly forecast file is available.

**Cross-reference joins added** (`core/cross_reference.py`): `thiess_neo`/`thiess_lao`
fill ComponentCode/ModifierCode from `thiess_cross-reference.csv`, keyed on (bare
Model, Service Type) — neither Supply Plan nor Component History carries either code
directly. `macmahon_neo`/`macmahon_lao` fill from `macmahon_cross-reference.csv`, keyed
on (Functional Location suffix, Position Code) — Component Code is already a real
column on Macmahon's Export sheet (refined where blank) but Modifier Code has no source
column at all and is entirely supplied by this join. Both follow the project's existing
"leave genuinely ambiguous keys blank rather than guess" convention
(`_index_csv_composite_unique`, the composite-key sibling of `_index_csv_unique_by`).

**Verified runs** (deterministic-only, `USE_LLM=false` — no LLM key configured in this
environment; same convention this README already uses for its other smoke-test
numbers):

- **Thiess+MACA**: Supply Plan (488 raw rows) → **NEO 32 delivered** (456 rows removed
  for missing ComponentCode+ModifierCode — the supplied 990-row cross-reference table
  doesn't cover every Model/Service-Type combination this Supply Plan snapshot
  forecasts); Component History (11,481 raw rows) → **LAO 1,999 delivered**. Every
  delivered row carries a real, cross-reference-sourced ComponentCode and ModifierCode
  (100% — the blank-pair rule removes anything the join couldn't resolve, so nothing
  unfilled ships). `cross_reference_sources`: `{"thiess_cross-reference.csv": "local"}`.
- **Macmahon**: Export (2,993 raw rows) → **NEO 2,658 delivered** (212 removed as
  older-dated duplicates via `deduplication.NEO`, 122 removed for missing both codes) /
  **LAO 2,870 delivered**. `cross_reference_sources`:
  `{"macmahon_cross-reference.csv": "local"}`.

**Known caveats** (documented in full in each `prompts/appendix.<id>.md`): these row
counts won't match the historical, manually-produced Upload CSVs in test-data — those
went through additional Excel-based curation (blank/1900-date cleanup, human exception
review, month-to-month carry-forward per the `Maca Workflow.png`-style process
diagrams) that this pipeline's own automated rules don't attempt to replicate
row-for-row, the same caveat this README already carries for FMG/Rio Tinto (§12/§13).
`BranchCode`/`SiteCode`/`FleetCode`/`CustomerCode`/`RegistrationCounter` stay blank for
all three new customers — Supply Plan/Component History/Export carry no compound
site/fleet/entity-name column, and `RegistrationCounter` in particular is not a fixed
per-customer constant here (it varies per fleet even within one `CustomerCode`), so
