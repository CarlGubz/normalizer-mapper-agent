...
directly) and a deterministic join against the AMT lookup tables in
`prompts/cross-references/*.csv` (never the LLM, never a guess). See
`prompts/README.md` § "AMT cross-reference enrichment" for exactly how, and
`core/cross_reference.py` for the per-workbook-shape join logic. No other Snowflake
logic is included, and every other enrichment field stays blank exactly as before.
